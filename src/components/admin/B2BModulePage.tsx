...
const configs: Record<string, ModuleConfig> = {
  "blog-center": {
    ...
    columns: [{ key: "title", label: "Title" }, { key: "featured_image", label: "Image" }, { key: "content_type", label: "Type" }, { key: "target_country", label: "Target Country" }, { key: "reading_time", label: "Reading Time" }, { key: "primary_keyword", label: "Primary Keyword" }, { key: "seo_score", label: "SEO Score" }, { key: "geo_score", label: "GEO Score" }, { key: "status", label: "Status" }, { key: "updated_at", label: "Updated" }],
    fields: [{ key: "title", label: "Title", required: true, full: true }, { key: "slug", label: "Slug", required: true }, { key: "content_type", label: "Content Type", type: "select", options: ["blog", "article"] }, { key: "target_country", label: "Target Country" }, { key: "category", label: "Category" }, { key: "primary_keyword", label: "Primary Keyword" }, { key: "keywords", label: "Secondary Keywords", type: "array", full: true }, { key: "excerpt", label: "Excerpt", type: "textarea", full: true }, { key: "content", label: "Article Content", type: "textarea", full: true }, { key: "featured_image", label: "Featured Image URL", type: "file", full: true }, { key: "seo_title", label: "SEO Title", full: true }, { key: "seo_description", label: "SEO Description", type: "textarea", full: true }, { key: "seo_score", label: "SEO Score", type: "number" }, { key: "geo_score", label: "GEO Score", type: "number" }, { key: "reading_time", label: "Reading Time" }, { key: "image_prompt", label: "Featured Image Prompt", type: "textarea", full: true }, { key: "internal_links", label: "Internal Link Suggestions", type: "json", full: true }, { key: "status", label: "Status", type: "select", options: ["draft", "review", "approved", "scheduled", "published", "archived"] }, { key: "published_at", label: "Publish Date", type: "datetime" }],
...
export default function B2BModulePage({ moduleKey }: { moduleKey: string }) {
  ...
  const automaticResearchAttempted = useRef(false);
  ...
  useEffect(() => {
    ...
    if ((action === "generate" || action === "research") && ["blog-center", "faq-intelligence", "outreach", "competitors"].includes(moduleKey)) void runAutomation();
  ...
  useEffect(() => {
    if (loading || automaticResearchAttempted.current || !["blog-center", "faq-intelligence"].includes(moduleKey)) return;
    const today = new Date().toISOString().slice(0, 10);
    const hasToday = rows.some(row => String(row.created_at || row.updated_at || "").slice(0, 10) === today);
    if (!hasToday) { automaticResearchAttempted.current = true; void runAutomation(); }
  }, [loading, rows, moduleKey]);
  ...
  return <div className="os-page">
    ...
    {moduleKey === "blog-center" && <section className="os-card blog-automation-workspace"><div className="os-card-header"><div><h2>Today’s Automated Publishing Desk</h2><p>Daily web research prepares one buyer-help blog and one SEO article with keywords, metadata, internal links and a featured image. Human approval remains mandatory.</p></div><span className="os-badge green">Vercel Cron · 06:00 UTC</span></div><div className="os-card-body"><div className="blog-automation-strip"><article><span>Research</span><strong>OpenAI Web Search</strong><small>Trending buyer intent and export topics</small></article><article><span>Content Pack</span><strong>2 Drafts Daily</strong><small>Blog + SEO article</small></article><article><span>Image</span><strong>Auto Generated</strong><small>Saved to CMS Media</small></article><article><span>Approval</span><strong>Review → Approve → Publish</strong><small>Never auto-published</small></article></div>{todaysBlogRows.length ? <div className="today-blog-grid">{todaysBlogRows.map(row => <article className="today-blog-card" key={String(row.id)}>{row.featured_image ? <img src={String(row.featured_image)} alt=""/> : <div className="today-blog-image-placeholder"><Sparkles/><span>Image pending API connection</span></div>}<div className="today-blog-copy"><div className="today-blog-meta"><span>{titleCase(row.content_type || "blog")}</span><span className={`os-badge ${toneForStatus(row.status)}`}>{titleCase(row.status)}</span></div><h3>{formatValue(row.title)}</h3><p>{formatValue(row.excerpt).slice(0, 180)}</p><div className="today-blog-scores"><span>SEO <b>{Number(row.seo_score || 0)}</b></span><span>GEO <b>{Number(row.geo_score || 0)}</b></span><span>Keyword <b>{formatValue(row.primary_keyword)}</b></span></div><div className="os-row-actions"><button className="os-btn primary" onClick={() => openBlogReview(row)}><BookOpen/>Review Full Blog</button><button className="os-btn soft" onClick={() => openEdit(row)}><Edit3/>Edit Fields</button></div></div></article>)}</div> : <div className="blog-automation-empty">{automationRunning ? <><RefreshCw className="animate-spin"/><strong>Researching today’s topics and preparing drafts…</strong></> : <><Sparkles/><strong>No automated draft exists for today.</strong><button className="os-btn primary" onClick={() => void runAutomation()}>Generate Today’s Drafts</button></>}</div>}</div></section>}
