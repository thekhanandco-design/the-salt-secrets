    ...
    <AdminShell>
      <div className="os-page legacy-unified-page images-manager-page space-y-5">
        <div className="flex flex-col xl:flex-row xl:items-end xl:justify-between gap-4">
          <div><p className="text-blue-400 uppercase tracking-[4px] text-xs font-black">Visual Content</p><h1 className="cms-page-title">Images Manager</h1></div>
          ...
          <div className="grid lg:grid-cols-[220px_1fr] min-h-[680px]">
            <aside className="image-manager-sidebar border-r p-4">
              ...
              <div className="space-y-1">
                {pages.map((page) => <button key={page} onClick={()=>setActivePage(page)} className={`image-page-button w-full flex items-center justify-between rounded-xl px-3 py-3 text-left text-sm font-bold ${activePage===page?"active":""}`}><span>{cmsPageLabels[page] || page}</span><span className="rounded-md bg-black/20 px-2 py-0.5 text-xs">{counts[page]}</span></button>)}
              </div>
              <div className="image-upload-note mt-7 rounded-2xl border p-4 text-xs leading-6"><p className="font-black text-blue-300 mb-2">Image upload</p>JPG, PNG or WebP<br/>Maximum 5MB<br/>Recommended size appears on every card.</div>
            </aside>
            ...
            <section className="p-4 lg:p-6">
              <div className="flex items-center justify-between mb-5">
                ...
                <div className="flex rounded-xl border border-white/10 overflow-hidden"><button onClick={()=>setView("grid")} className={`p-3 ${view==="grid"?"cms-gradient-button text-white":""}`}><Grid2X2 className="w-4 h-4"/></button><button onClick={()=>setView("list")} className={`p-3 ${view==="list"?"cms-gradient-button text-white":""}`}><List className="w-4 h-4"/></button></div>
              </div>
              ...
              <div className={view === "grid" ? "grid md:grid-cols-2 xl:grid-cols-3 gap-4" : "space-y-4"}>
                {visible.map((slot) => {
                  ...
                  return <article key={key} className={`image-slot-card rounded-2xl border p-3 ${view === "list" ? "grid md:grid-cols-[220px_1fr_auto] gap-5 items-center" : ""}`}>
                    <div className="image-preview aspect-[4/3] rounded-xl overflow-hidden flex items-center justify-center">{preview ? <img src={preview} alt={slot.alt_text || slot.title} className="w-full h-full object-contain"/> : <ImageIcon className="w-10 h-10 text-slate-600"/>}</div>
                    ...
                    <div className={`flex gap-2 ${view === "grid" ? "mt-3" : ""}`}><label className="cms-gradient-button flex-1 inline-flex justify-center items-center gap-2 rounded-xl px-4 py-3 text-xs font-black cursor-pointer text-white"><Upload className="w-4 h-4"/>{uploading===key?"Uploading...":"Replace Image"}<input type="file" accept="image/*" className="hidden" onChange={(e)=>replace(slot,e.target.files?.[0])}/></label><button onClick={()=>reset(slot)} title="Reset to original" className="rounded-xl border border-white/10 px-3 text-slate-300"><RefreshCw className="w-4 h-4"/></button></div>
                  </article>;
                })}
              </div>
            </section>
