...
export default function SocialStudio() {
  ...
  async function savePost(targetStatus = "Draft") {
    ...
    const statusValue = targetStatus === "Approved" ? "scheduled" : normal(targetStatus).replaceAll(" ", "_");
    ...
    const payload = {
      ...
      platform_results: {}, last_error: null, updated_at: new Date().toISOString(), brief,
    };
  ...
  async function updateStatus(row: SocialRow, nextStatus: string) {
    const result = await supabase.from("social_scheduled_posts").update({ approval_status: nextStatus, status: nextStatus === "Approved" ? "scheduled" : normal(nextStatus).replaceAll(" ", "_"), approved_at: nextStatus === "Approved" ? new Date().toISOString() : row.approved_at, updated_at: new Date().toISOString() }).eq("id", row.id);
  ...
  return <AdminShell><div className="os-page">
    ...
    {activeTab === "Create Post" ? <div className="os-three-panel">
      ...
      <main className="os-card"><div className="os-card-header"><div><h2>Platform Content Editor</h2><p>Each platform has separate editable content</p></div><span className={`os-badge ${active.status === "Approved" ? "green" : active.status === "Needs Review" ? "amber" : "blue"}`}>{active.status}</span></div><div className="os-card-body"><div className="os-tabs" style={{ marginBottom: 14 }}>{platformKeys.map(platform => <button key={platform} className={`os-tab ${activePlatform === platform ? "active" : ""}`} onClick={() => setActivePlatform(platform)}><PlatformLogo platform={platform}/>{platformMeta[platform].label}</button>)}</div>
        ...
        <label className="os-label" style={{ marginTop: 12 }}><span>{activePlatform === "instagram" ? "Caption" : activePlatform === "youtube" ? "Description" : "Post Copy"}</span><textarea style={{ minHeight: 210 }} value={active.text} onChange={event => patchDraft({ text: event.target.value })}/><small className={`social-char-counter ${active.text.length > meta.maxChars ? "over" : ""}`}><span>{active.text.length.toLocaleString()} / {meta.maxChars.toLocaleString()} max</span><span>Recommended ~{meta.recommendedChars.toLocaleString()} characters</span></small></label>
