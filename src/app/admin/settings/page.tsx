...
export default function SettingsPage() {
  ...
  async function uploadLogo(file?: File) {
    if (!file) return;
    setSaving(true); setError("");
    const path=`brand/${Date.now()}-${file.name.replace(/[^a-zA-Z0-9._-]/g,"-")}`;
    const upload=await supabase.storage.from("cms-media").upload(path,file,{upsert:true});
    ...
    update("app_icon_url",url); setSaving(false); setToast("Logo uploaded. Save Changes to apply it.");
  }
  ...
  return <AdminShell><div className="os-page">
    ...
    {toast&&<div className="os-toast-stack"><div className="os-toast"><span className="os-toast-icon"><CheckCircle2/></span><div><strong>{toast}</strong><span>No placeholder record was created.</span></div></div></div>}
  </div></AdminShell>;
}
...
function IntegrationPanel({ids,statuses}:{ids:string[];statuses:Record<string,LiveStatus>}) { return <div className="os-grid two">{ids.map(id=>{const status=statuses[id];const external=status?.mode==="external";const configured=Boolean(status?.configured&&!external);return <article className="os-card" style={{boxShadow:"none"}} key={id}><div className="os-card-body" style={{display:"flex",alignItems:"center",gap:12}}><span className="os-list-icon"><Link2/></span><div className="os-list-main"><strong>{integrationLabels[id]||id}</strong><span>{configured?"Configured":external?"External Tool":`Connection Required${status?.missing?.length?`: ${status.missing.join(", ")}`:""}`}</span></div><Link className="os-btn soft" href="/admin/integrations">{configured?"Test":"Configure"}</Link></div></article>;})}</div>; }
...
function SecurityPanel({config,updateConfig}:{config:Record<string,unknown>;updateConfig:(key:string,value:unknown)=>void}) {
  ...
  return <div className="os-list"><div className="os-list-row"><span className="os-list-icon"><KeyRound/></span><div className="os-list-main"><strong>Authenticated account</strong><span>{email||"No authenticated account found"}{lastSignIn?` · Last sign in ${new Date(lastSignIn).toLocaleString()}`:""}</span></div></div><div className="os-list-row"><span className="os-list-icon"><ShieldCheck/></span><div className="os-list-main"><strong>Multi-factor authentication</strong><span>Managed through Supabase Auth. The CMS does not display a false enabled state.</span></div><a className="os-btn soft" href="https://supabase.com/dashboard" target="_blank" rel="noreferrer">Open Auth Settings</a></div><div className="os-list-row"><div className="os-list-main"><strong>Session timeout preference</strong><span>Stored in CMS settings for application enforcement.</span></div><select className="os-field" value={textValue(config.sessionTimeout)||"8"} onChange={event=>updateConfig("sessionTimeout",event.target.value)}><option value="2">2 hours</option><option value="8">8 hours</option><option value="24">24 hours</option></select></div></div>;
}
