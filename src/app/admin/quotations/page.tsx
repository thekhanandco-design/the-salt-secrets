...
type DocumentForm = {
  ...
  company_address: string; company_website: string; authorized_by: string; authorized_title: string;
...
const documentTypes = [
  ...
  ["sample_invoice", "Sample Invoice"], ["coa_cover", "COA Cover Document"],
];
...
const blankItem = (): LineItem => ({ product: "", export_reference: "", specification: "", packaging: "", moq: "", quantity: 0, unit: "MT", unit_price: 0, origin: "Pakistan", hs_code: "" });
const blankForm = (): DocumentForm => ({
  ...
  company_address: "", company_website: "", authorized_by: "", authorized_title: "", freight: 0, discount: 0, insurance: 0,
...
function documentHint(type: string) {
  ...
  return "The form and A4 preview adapt to the selected export document type.";
}
...
export default function QuotationsPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (letterheads.data) {
      ...
      setForm(current => ({ ...current, company_name: head.company_name || current.company_name, company_email: head.email || "", company_phone: head.phone || "", company_address: head.address || "", company_website: head.website || "" }));
    }
    setLoading(false);
  }, []);
    ...
    {view === "dashboard" ? <>
      ...
      <div className="quotation-builder-grid">
        <div className="quotation-form-column">
          ...
          <EditorSection number="4" title="Commercial Terms"><div className="quotation-form-grid"><Field label="Payment Terms" value={form.payment_terms} onChange={value => patch("payment_terms", value)}/><Field label="Delivery Timeline" value={form.delivery_terms} onChange={value => patch("delivery_terms", value)}/><Field label="Shipment Method" value={form.shipment_method} onChange={value => patch("shipment_method", value)}/><Field label="Certifications / Attachments" value={form.certifications.join(", ")} onChange={value => patch("certifications", value.split(",").map(item => item.trim()).filter(Boolean))}/><label className="span-2"><span>Notes / Remarks</span><textarea value={form.notes} onChange={event => patch("notes", event.target.value)} rows={4}/></label></div></EditorSection>
...
function DocumentPreview({ form, totals }: { form: DocumentForm; totals: { subtotal: number; tax: number; grandTotal: number } }) {
  ...
  return <article id="quotation-print" className="a4-document a4-khan-document">
    ...
    <table className="a4-khan-table"><thead><tr><th>Sr#</th><th>Product Description</th><th>Specification</th><th>MOQ</th><th>Quantity</th><th>Unit</th><th>Packaging</th><th>Unit Price</th><th>Total</th></tr></thead><tbody>{form.items.map((item, index) => <tr key={index}><td>{index + 1}</td><td><strong>{item.product || ""}</strong>{item.export_reference ? <small>{item.export_reference}</small> : null}</td><td>{item.specification || ""}</td><td>{item.moq || ""}</td><td>{Number(item.quantity || 0).toLocaleString()}</td><td>{item.unit}</td><td>{item.packaging || ""}</td><td>{money(item.unit_price, form.currency)}</td><td>{money(item.quantity * item.unit_price, form.currency)}</td></tr>)}</tbody></table>
