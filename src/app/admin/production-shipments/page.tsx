...
import { AlertTriangle, Edit3, ExternalLink, Globe2, MapPinned, Maximize2, PackageSearch, Plus, RefreshCw, Search, Ship, Trash2, Truck, X } from "lucide-react";
...
export default function Shipments() {
  ...
  return <AdminShell><div className="os-page"><header className="os-page-header"><div><div className="os-page-eyebrow">Live export operations</div><h1 className="os-page-title">Production & Shipments</h1><p className="os-page-subtitle">Create, edit and track real production stages, export documents, carriers, ETD, ETA and delivery progress.</p></div><div className="os-page-actions"><button className="os-btn soft" onClick={() => void load()} disabled={loading}><RefreshCw className={loading ? "animate-spin" : ""}/>Refresh</button><button className="os-btn primary" onClick={startCreate}><Plus/>Add Shipment</button></div></header>
    {error && <section className="os-card" style={{ borderColor: "rgba(239,68,68,.35)" }}><div className="os-card-body" style={{ display: "flex", gap: 12 }}><AlertTriangle/><div><strong>Live shipment data error</strong><p className="os-page-subtitle">{error}</p><p className="os-page-subtitle">Run <b>supabase/THE-SALT-ORIGIN-LIVE-CMS.sql</b> if the new B2B fields are missing. No placeholder shipment records are shown.</p></div></div></section>}
    <div className="shipment-metrics-grid">{[["Production Orders", rows.filter(row => !["In Transit","Customs","Delivered"].includes(String(row.current_stage))).length, PackageSearch], ["Ready to Dispatch", rows.filter(row => normal(row.current_stage)==="ready for dispatch").length, Truck], ["In Transit", rows.filter(row => normal(row.current_stage)==="in transit").length, Ship], ["Arriving Soon", rows.filter(row => row.eta && new Date(row.eta).getTime()-Date.now() > 0 && new Date(row.eta).getTime()-Date.now() < 7*86400000).length, Globe2], ["Docs Pending", rows.filter(row => normal(row.document_status)!=="complete").length, ExternalLink], ["Completed", rows.filter(row => normal(row.current_stage || row.current_status)==="delivered").length, PackageSearch]].map(([label,value,Icon])=>{const Component=Icon as typeof Ship;return <article className="os-metric" key={String(label)}><div className="os-metric-top"><span className="os-metric-label">{String(label)}</span><span className="os-metric-icon"><Component/></span></div><div className="os-metric-value">{String(value)}</div><div className="os-metric-foot"><b>Operations</b></div></article>})}</div>
    <div className="shipment-overview-grid"><section className="os-card"><div className="os-card-header"><div><h2>Order Journey</h2><p>Production → QA → documents → dispatch → transit → delivery.</p></div><span className="os-badge green">WORKFLOW READY</span></div><div className="os-card-body shipment-journey-list">{[["01","Production Planning","Confirm product, volume and packaging."],["02","Quality & Packing","Complete batch and packaging checks."],["03","Export Documents","Prepare invoice, packing list and required documents."],["04","Shipment Tracking","Carrier, booking, ETD, ETA and delivery status."]].map(([n,t,c])=><article key={n}><span>{n}</span><div><strong>{t}</strong><p>{c}</p></div><b>Ready</b></article>)}</div></section><section className="os-card shipment-map-card"><div className="os-card-header"><div><h2>Live Shipment Map</h2><p>Carrier-map integration appears here when location data is available.</p></div><button className="os-icon-button" onClick={()=>setMapOpen(true)} title="Full screen"><Maximize2/></button></div><div className="os-card-body"><div className="shipment-map-stage"><img src="/world-map.png" alt="Shipment map"/><div className="shipment-map-status"><MapPinned/><strong>{active.length?`${active.length} active shipment${active.length===1?"":"s"}`:"No active shipments"}</strong><span>{active.some(row=>Boolean(trackingUrl(row)))?"Open carrier tracking from the shipment register for live provider status.":"Connect a carrier/location API for real-time map coordinates."}</span></div></div></div></section></div>
    ...
    {mapOpen && <div className="os-modal-backdrop" onMouseDown={()=>setMapOpen(false)}><section className="os-modal shipment-map-modal" onMouseDown={event=>event.stopPropagation()}><div className="os-modal-header"><div><h2>Shipment Tracking Map</h2><p className="os-page-subtitle">Real carrier links remain the source of live location until a map-position API is connected.</p></div><button className="os-icon-button" onClick={()=>setMapOpen(false)}><X/></button></div><div className="os-modal-body"><div className="shipment-map-stage fullscreen"><img src="/world-map.png" alt="Full shipment map"/></div><div className="shipment-map-list">{active.map(row=>{const url=trackingUrl(row);return <article key={row.id}><div><strong>{row.reference_number}</strong><span>{row.client_name||row.customer_accounts?.company_name||"Client"} · {row.origin||"Pakistan"} → {row.destination||row.country||"Destination"}</span></div><span className="os-badge blue">{nice(row.current_stage||row.current_status)}</span>{url?<a className="os-btn soft" href={url} target="_blank" rel="noreferrer"><ExternalLink/>Live Carrier Tracking</a>:<span className="os-badge amber">Tracking link required</span>}</article>})}</div></div></section></div>}
    {open && <div className="os-modal-backdrop" onMouseDown={() => setOpen(false)}><section className="os-modal" style={{ width: "min(1050px,96vw)", maxHeight: "92vh", overflow: "auto" }} onMouseDown={event => event.stopPropagation()}><div className="os-modal-header"><div><h2>{editingId ? "Edit Shipment" : "Add Shipment"}</h2><p className="os-page-subtitle">Saved directly to the live shipment register.</p></div><button className="os-icon-button" onClick={() => setOpen(false)}><X/></button></div><div className="os-modal-body"><div className="os-form-grid">
