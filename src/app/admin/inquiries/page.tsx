...
const statuses = ["new_inquiry","contacted","qualified","warm_lead","sample_requested","sample_sent","quotation_requested","quotation_sent","quotation_viewed","follow_up_due","negotiation","waiting_reply","client","future_opportunity","dormant","lost"];
const statusMeta: Record<string, { label: string; cls: string; Icon: typeof Clock3 }> = {
  new: { label: "New Inquiry", cls: "bg-blue-500/10 text-blue-500 border-blue-500/20", Icon: Clock3 },
  new_inquiry: { label: "New Inquiry", cls: "bg-blue-500/10 text-blue-500 border-blue-500/20", Icon: Clock3 },
  contacted: { label: "Contacted", cls: "bg-amber-500/10 text-amber-500 border-amber-500/20", Icon: CheckCircle2 },
  qualified: { label: "Qualified", cls: "bg-violet-500/10 text-violet-500 border-violet-500/20", Icon: CheckCircle2 },
  warm_lead: { label: "Warm Lead", cls: "bg-orange-500/10 text-orange-500 border-orange-500/20", Icon: Trophy },
  sample_requested: { label: "Sample Requested", cls: "bg-cyan-500/10 text-cyan-600 border-cyan-500/20", Icon: FileCheck2 },
  sample_sent: { label: "Sample Sent", cls: "bg-cyan-500/10 text-cyan-600 border-cyan-500/20", Icon: FileCheck2 },
  quotation_requested: { label: "Quotation Requested", cls: "bg-indigo-500/10 text-indigo-500 border-indigo-500/20", Icon: FileCheck2 },
  quotation_sent: { label: "Quotation Sent", cls: "bg-emerald-500/10 text-emerald-500 border-emerald-500/20", Icon: FileCheck2 },
  quotation_viewed: { label: "Quotation Viewed", cls: "bg-emerald-500/10 text-emerald-600 border-emerald-500/20", Icon: FileCheck2 },
  follow_up_due: { label: "Follow-Up Due", cls: "bg-rose-500/10 text-rose-500 border-rose-500/20", Icon: Clock3 },
  negotiation: { label: "Negotiation", cls: "bg-fuchsia-500/10 text-fuchsia-500 border-fuchsia-500/20", Icon: Trophy },
  waiting_reply: { label: "Waiting Reply", cls: "bg-slate-500/10 text-slate-600 border-slate-500/20", Icon: Clock3 },
  client: { label: "Client", cls: "bg-green-500/10 text-green-600 border-green-500/20", Icon: Trophy },
  future_opportunity: { label: "Future Opportunity", cls: "bg-blue-500/10 text-blue-600 border-blue-500/20", Icon: Clock3 },
  dormant: { label: "Dormant", cls: "bg-slate-500/10 text-slate-500 border-slate-500/20", Icon: Clock3 },
  won: { label: "Client", cls: "bg-green-500/10 text-green-500 border-green-500/20", Icon: Trophy },
...
export default function InquiriesPage() {
  ...
  function exportCsv() {
    const rows = [["Date","Name","Email","Company","WhatsApp","Country","Product","Quantity","Status","Message"], ...inquiries.map((item) => [item.created_at,item.name,item.email,item.company,item.whatsapp,item.country,item.product,item.quantity,item.status || "new_inquiry",item.message])];
  ...
  const filteredInquiries = useMemo(() => {
    ...
    return inquiries.filter((item) => {
      ...
      const statusOk = statusFilter === "all" || (item.status || "new_inquiry") === statusFilter;
  ...
  const newCount = inquiries.filter((item) => !item.status || item.status === "new" || item.status === "new_inquiry").length;

  return <AdminShell><div className="space-y-8">
    <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4"><div><p className="uppercase tracking-[5px] text-[#C23B4A] font-black text-xs">CRM</p><h1 className="text-4xl lg:text-5xl font-black mt-2">Inquiries Dashboard</h1><p className="text-slate-500 mt-3">Manage every B2B inquiry through qualification, samples, quotation, negotiation and client conversion.</p></div><button onClick={exportCsv} className="bg-[#C23B4A] text-white px-6 py-4 rounded-xl font-black w-fit">Export CSV</button></div>
