...
export const metadata: Metadata = {
  title: "Himalayan Pink Salt Articles | The Salt Origin",
...
export default async function BlogPage() {
  const { data } = await supabase.from("blog_posts").select("id,title,slug,excerpt,featured_image,published_at,created_at,content_type").eq("status", "published").eq("content_type","article").order("published_at", { ascending: false });
    ...
    <main className="bg-[#FFF8F5] min-h-screen">
      <section className="max-w-[1450px] mx-auto px-6 lg:px-12 py-20">
        <div className="text-center max-w-4xl mx-auto">
          ...
          <h1 className="mt-4 text-[#081325] font-black" style={{ fontFamily: "Georgia, serif", fontSize: "clamp(2.8rem,5vw,5rem)" }}>The Salt Origin Articles</h1>
          <p className="mt-5 text-lg text-slate-600">Long-form sourcing, quality, logistics and private-label guidance for professional buyers.</p>
        </div>
        ...
        ) : <div className="mt-14 rounded-[28px] bg-white p-10 text-center border border-[#EFE3E5]">No published articles yet.</div>}

        {older.length > 0 && <>
          <div className="flex items-center gap-4 mt-16"><span className="h-px flex-1 bg-[#E8C8CD]" /><h2 className="text-2xl font-black text-[#081325]">More Articles</h2><span className="h-px flex-1 bg-[#E8C8CD]" /></div>
